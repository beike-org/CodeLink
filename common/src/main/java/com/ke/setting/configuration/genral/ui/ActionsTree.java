...
public final class ActionsTree {
	...
	private Group myMainGroup = new Group("", null, (Icon) null);
	...
	public void reset(@NotNull Keymap keymap) {
		myKeymap = keymap;
		...
		Group mainGroup = createPluginsActionsGroup();
	...
	private class MyModel extends DefaultTreeModel implements TreeTableModel {
		...
		public String getColumnName(int column) {
			switch (column) {
				case 0:
					return KeyMapBundle.message("action.column.name");
				case 1:
					return KeyMapBundle.message("shortcuts.column.name");
			}
			return "";
		}
	...
	private class KeymapsRenderer extends ColoredTreeCellRenderer {
		...
		// Make sure that the text rendered by this method is 'searchable' via filter method.
		@Override
		public void customizeCellRenderer(@NotNull JTree tree, Object value, boolean selected, boolean expanded, boolean leaf, int row, boolean hasFocus) {
			...
			if (value instanceof DefaultMutableTreeNode) {
				...
				if (!myHaveLink) {
					...
					SearchUtil.appendFragments(null, text, SimpleTextAttributes.STYLE_PLAIN, foreground, background, this);
		...
		private String getActionText(@Nullable AnAction action, String actionId) {
			...
			if (text == null || text.isEmpty()) { //fill dynamic presentation gaps
				text = actionId;
			}
			return text;
		}
		...
		public void append(@NotNull String fragment, @NotNull SimpleTextAttributes attributes, Object tag) {
			if (tag instanceof Hyperlink && !(tag instanceof KeymapsRenderer.ActionHyperlink)) {
				myHaveLink = true;
		...
		public AccessibleContext getAccessibleContext() {
			if (accessibleContext == null) {
				accessibleContext = new KeymapsRenderer.AccessibleKeymapsRenderer();
			}
		...
		private class ActionHyperlink extends Hyperlink {
			...
			ActionHyperlink(String actionId, String actionText) {
	...
	private static Group createPluginsActionsGroup() {
		Group pluginsGroup = new Group(KeyMapBundle.message("plugins.group.title"), null, (Icon) null);

		ActionManagerEx actionManager = ActionManagerEx.getInstanceEx();
		PluginId pluginId = PluginUtil.PLUGIN_ID;
		String[] pluginActions = actionManager.getPluginActions(pluginId);

		//noinspection HardCodedStringLiteral
		String name = StringUtil.notNullize("CodeLink", pluginId.getIdString());
		Group pluginGroup = createPluginActionsGroup(name, pluginActions);
	...
	@NotNull
	private static Group createPluginActionsGroup(String name, String[] pluginActions) {
		ActionManagerEx actionManager = ActionManagerEx.getInstanceEx();
		Group pluginGroup = new Group(name, null, (Icon) null);
		...
		for (String actionId : pluginActions) {
			...
			// 过滤掉不可执行的ActionGroup和webview的调试Action
			if (isNonExecutableActionGroup(actionId, action) || "myWebviewDebugAction".equals(actionId)) {
				continue;
			}
